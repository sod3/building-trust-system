  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white flex flex-col items-center justify-center px-4 py-12">
        ...
        <h1 className="font-display text-3xl font-semibold text-center text-foreground">Work Submitted!</h1>
        <p className="mt-3 text-center text-muted-foreground max-w-sm text-lg">
          Today's work submitted successfully.<br />
        ...
        <div className="mt-8 w-full max-w-sm rounded-3xl border-2 border-emerald-200 bg-white p-6 shadow-elevated">
          <div className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 text-center">Today's Summary</div>
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-3 text-center">
              ...
              <div className="text-xs text-emerald-600 mt-0.5 font-medium">Done</div>
            </div>
            <div className="rounded-2xl bg-amber-50 border border-amber-200 p-3 text-center">
              ...
              <div className="text-xs text-amber-600 mt-0.5 font-medium">Pending</div>
            </div>
            <div className="rounded-2xl bg-sky-50 border border-sky-200 p-3 text-center">
              ...
              <div className="text-xs text-sky-600 mt-0.5 font-medium">Time</div>
            </div>
          </div>

          <div className="mt-4 text-center text-sm text-muted-foreground">
            Building: <strong>{labour.buildingName}</strong>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        {/* Greeting */}
        <div className="mb-6">
          <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-1">Today's Work</div>
          <h1 className="font-display text-3xl font-semibold text-foreground">
            Good Morning, {labour.name.split(" ")[0]} 👋
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">{today}</p>
          <p className="mt-0.5 text-sm text-muted-foreground">Tick each task when completed, then submit your daily work.</p>
        </div>
        ...
        <div className="mb-6 rounded-3xl bg-navy-gradient text-white p-5 shadow-elevated">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="font-display text-2xl font-semibold mt-0.5">{completedCount} / {totalCount} Tasks</div>
            </div>
          ...
          {tasks.map((task, i) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-4 rounded-2xl border-2 p-4 text-left transition-all duration-300 active:scale-[0.99] hover:shadow-md",
                  done
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className={cn("text-sm mt-0.5", done ? "text-emerald-600" : "text-muted-foreground")}>
                    {done ? "✓ Completed" : task.instruction}
                  </div>
          ...
          >
            {completedCount === totalCount ? (
              <span className="flex items-center justify-center gap-2">
                <CheckCheck className="h-6 w-6" /> Submit Today's Work
              </span>
            ) : (
              <span>Submit Today's Work ({completedCount}/{totalCount} done)</span>
            )}
          </button>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-3xl bg-white p-6 shadow-elevated">
            ...
            <h3 className="font-display text-xl font-semibold text-center">Pending Tasks</h3>
            <p className="mt-2 text-center text-muted-foreground text-sm">
              <strong>{totalCount - completedCount} tasks</strong> are still pending.
