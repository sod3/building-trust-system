...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!building || building.status === "deleted")
      return sendError(res, 404, "Building not found.");
    ...
    if (req.method === "DELETE") {
      await context.db
        ...
        .updateOne({ _id: building._id }, { $set: { status: "deleted", updatedAt: new Date() } });
      await writeAuditLog(context.db, {
        orgId: building.orgId,
        userId: user._id,
        action: "building.deleted",
        details: { buildingId: building._id },
      });
    ...
    sendError(
      res,
      error.status || 500,
      error.status ? error.message : "Could not process building.",
    );
  }
}
