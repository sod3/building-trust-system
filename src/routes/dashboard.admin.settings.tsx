  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <Card>
        ...
        <label className="mb-1.5 block text-sm font-medium text-foreground">{t("admin.settings.platform_name", { fallback: "Platform Name" })}</label>
        <input value={platformName} onChange={(event) => setPlatformName(event.target.value)} className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20" />
        <div className="mt-4 rounded-xl border border-border bg-secondary/50 px-4 py-3">
          <div className="mb-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("admin.settings.notice_title", { fallback: "Admin Access Notice" })}</div>
      ...
      <Card>
        <div className="mb-4 flex items-center gap-3">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-navy/10">
            <span className="text-sm font-bold text-navy">{adminName.split(" ").map((part) => part[0]).join("").slice(0, 2) || "PA"}</span>
          </div>
          <h3 className="font-display font-semibold">{t("admin.settings.profile", { fallback: "Admin Profile" })}</h3>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">{t("common.full_name", { fallback: "Full Name" })}</label>
            <input value={adminName} onChange={(event) => setAdminName(event.target.value)} className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">{t("common.email", { fallback: "Email Address" })}</label>
            <input value={adminEmail} onChange={(event) => setAdminEmail(event.target.value)} type="email" className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20" />
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={item.key} className="flex items-center justify-between border-b border-border py-2 last:border-0">
      ...
      <Card>
        ...
        <select value={language} onChange={(event) => setLanguage(event.target.value)} className="h-10 w-full max-w-xs rounded-xl border border-border bg-background px-3 text-sm outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">Persistence can be connected to a platform settings API when needed.</span>
      </div>
