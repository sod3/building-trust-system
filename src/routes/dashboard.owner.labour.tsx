...
function OwnerLabour() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", email: "", password: "", buildingId: "" });
  ...
  async function handleAdd() {
    try {
      ...
      showToast(result.temporaryPassword ? `Labour added. Temporary password: ${result.temporaryPassword}` : "Labour added successfully.");
      load();
    } catch (error) {
  ...
  const labourLimitReached = usage.labourLimit !== null && usage.labourLimit !== undefined && usage.labourUsersUsed >= usage.labourLimit;
    ...
    <div className="space-y-6">
            ...
            <Card key={worker.id} className="hover:shadow-elevated transition-all">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5"><StatusPill status={worker.todayStatus} /></div>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{worker.completedTasksToday}/{worker.totalTasksToday}</span>
                </div>
                <ProgressBar
                  value={worker.completedTasksToday}
                  max={worker.totalTasksToday || 1}
                  color={worker.completedTasksToday === worker.totalTasksToday && worker.totalTasksToday ? "emerald" : worker.completedTasksToday > 0 ? "amber" : "rose"}
                />
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("owner.labour.last_active", { fallback: "Last active" })}</span>
                  <span>{worker.lastSubmission === "Never" ? "Never" : new Date(worker.lastSubmission).toLocaleString()}</span>
                </div>
              </div>

              <div className="mt-4 flex gap-2">
                <Btn size="sm" variant="outline" className="flex-1" onClick={() => showToast(`${worker.name}'s checklist data is loaded from daily reports.`)}>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title={t("admin.labour.add", { fallback: "Add Labour" })}>
        <div className="space-y-4">
          <FormInput label={t("common.full_name", { fallback: "Full Name" })} value={form.name} onChange={(value) => setForm((current) => ({ ...current, name: value }))} placeholder="Ali Hassan" required />
          <FormInput label={t("common.phone", { fallback: "Phone" })} value={form.phone} onChange={(value) => setForm((current) => ({ ...current, phone: value }))} placeholder="+966 53 000 0000" required />
          <FormInput label={t("common.email", { fallback: "Email" })} value={form.email} onChange={(value) => setForm((current) => ({ ...current, email: value }))} placeholder="ali@example.com" type="email" required />
          <FormInput label="Password" value={form.password} onChange={(value) => setForm((current) => ({ ...current, password: value }))} placeholder="Leave blank to generate" type="password" />
          <FormSelect
            ...
            options={[{ value: "", label: "Select Building" }, ...buildings.map((building: any) => ({ value: building.id, label: building.name }))]}
          />
