...
function AdminBuildings() {
  ...
  const filtered = buildings.filter((building) => {
    ...
    return [building.name, building.city, building.address, orgName]
      .join(" ")
      .toLowerCase()
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative min-w-48 flex-1">
            ...
            <input
              placeholder={t("admin.buildings.search", {
                fallback: "Search by building name, city, or owner...",
              })}
              value={search}
          ...
          <div className="text-xs text-muted-foreground">
            {filtered.length} {t("common.buildings", { fallback: "buildings" })}
          </div>
        </div>
      </Card>
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState
            ...
            title={t("admin.buildings.no_records", { fallback: "No buildings found" })}
            body={t("common.no_records_body", { fallback: "Try adjusting your search." })}
          />
        </Card>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {orgMap.get(building.orgId)?.name || building.orgId}
                  </div>
                </div>
      ...
      >
        <div className="space-y-4">
          <FormSelect
            label="Organization"
            value={form.orgId}
            onChange={(value) => setForm((current) => ({ ...current, orgId: value }))}
            options={[
          ...
          <div className="flex justify-end gap-2">
            ...
            <Btn onClick={handleAdd} disabled={!form.orgId || !form.name}>
