...
export default async function handler(req, res) {
  ...
  try {
    ...
    const organizations = await db.collection("organizations").aggregate([
      { $lookup: { from: "users", localField: "ownerUserId", foreignField: "_id", as: "ownerRows" } },
      { $lookup: { from: "subscriptions", localField: "_id", foreignField: "orgId", as: "subscriptionRows" } },
      { $lookup: { from: "buildings", localField: "_id", foreignField: "orgId", as: "buildingRows" } },
    ...
    sendJson(res, 200, {
      ...
      organizations: organizations.map((org) => ({
        ...
        buildingCount: (org.buildingRows || []).filter((building) => building.status !== "deleted").length,
      })),
    });
  } catch (error) {
