...
export default async function handler(req, res) {
  ...
  try {
    ...
    sendJson(res, 200, { success: true, usage: dashboard.usage });
  } catch (error) {
    console.error("[owner-usage]", error.message);
    sendError(res, error.status || 500, error.status ? error.message : "Could not load usage.");
  }
}
