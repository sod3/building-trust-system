...
export function Kpi({
  ...
  if (accent) {
    return (
      <div
        className={cn(
          "relative overflow-hidden rounded-2xl bg-gradient-to-br p-5 shadow-elevated",
          accentMap[accent],
        )}
      >
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border bg-card p-5 transition hover:shadow-elevated">
      <div className="flex items-start justify-between">
        ...
        {icon && (
          <div className="grid h-8 w-8 place-items-center rounded-lg bg-secondary text-navy">
            {icon}
          </div>
        )}
      </div>
...
export function StatusPill({ status }: { status: string }) {
  ...
  const { t } = useLang();
  const displayStatus = t(`status.${status.toLowerCase().replace(/ /g, "_")}`, {
    fallback: status,
  });
  return (
    <span
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl bg-background border border-border shadow-elevated max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-lg hover:bg-secondary transition text-muted-foreground"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
...
export function Btn({
  ...
  const base =
    "inline-flex items-center justify-center gap-2 rounded-xl font-medium transition-all active:scale-[0.98] disabled:opacity-60";
...
export function Toast({
  ...
  return (
    <div
      className={cn(
        "fixed bottom-4 end-4 z-50 flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium shadow-elevated",
        styles[type],
      )}
    >
      {message}
...
export function AlertBanner({
  ...
  return (
    <div
      className={cn("flex items-center gap-2.5 rounded-xl border px-4 py-3 text-sm", styles[type])}
    >
