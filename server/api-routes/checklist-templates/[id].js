...
export default async function handler(req, res) {
  ...
  try {
    ...
    const filter =
      context.role === ROLES.SUPER_ADMIN ? { _id: id } : { _id: id, orgId: context.org._id };
    ...
    const update = {
      ...
      buildingId:
        body.buildingId === undefined ? template.buildingId : asString(body.buildingId) || null,
      updatedAt: new Date(),
    };
    if (body.items) {
      update.items = []
    ...
    await context.db
      ...
      .updateOne({ _id: template._id }, { $set: update });
    ...
    sendError(
      res,
      error.status || 500,
      error.status ? error.message : "Could not process checklist template.",
    );
  }
}
