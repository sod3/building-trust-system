...
function OwnerSettings() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <PageHeader
        title={t("dashboard.owner.nav.settings", { fallback: "Settings" })}
        subtitle={t("owner.settings.subtitle", {
          fallback: "Manage your profile and preferences.",
        })}
      />
      ...
      <Card>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              {planName} {t("common.owner", { fallback: "Owner" })}
            </div>
          </div>
        </div>
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">
                {t("common.company_name", { fallback: "Company Name" })}
              </label>
              <input
                value={company}
      ...
      <Card>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-display font-semibold">
            {t("admin.settings.notifications", { fallback: "Notifications" })}
          </h3>
        </div>
        <div className="space-y-3">
          {[
            {
              key: "email",
              label: t("admin.settings.email_notif", { fallback: "Email Notifications" }),
              desc: t("owner.settings.email_desc", { fallback: "Receive daily summary emails" }),
            },
            {
              key: "reports",
              label: t("owner.settings.report_alerts", { fallback: "Report Alerts" }),
              desc: t("owner.settings.report_desc", {
                fallback: "Get notified when labour submits reports",
              }),
            },
            {
              key: "overdue",
              label: t("owner.settings.overdue_alerts", { fallback: "Overdue Task Alerts" }),
              desc: t("owner.settings.overdue_desc", {
                fallback: "Alert if tasks are not completed by EOD",
              }),
            },
          ].map((item) => (
            <div
      ...
      <Card>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-display font-semibold">
            {t("admin.settings.language", { fallback: "Language" })}
          </h3>
        </div>
        <select
          value={language}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">
          Profile editing can be connected to a settings API when needed.
        </span>
      </div>
