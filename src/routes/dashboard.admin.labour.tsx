...
function AdminLabour() {
  ...
  const filtered = labour.filter((worker) =>
    [worker.name, worker.email, worker.phone, worker.buildingName]
      .join(" ")
      .toLowerCase()
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={t("admin.labour.subtitle", {
          fallback: "All labour workers and their daily activity across buildings.",
        })}
        actions={
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState
            ...
            body="Create labour users from an owner account or from this admin page."
          />
        </Card>
      ) : (
            ...
            <Card key={worker.id}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5">
                    <StatusPill status={worker.todayStatus || worker.status} />
                  </div>
                </div>
              </div>
              <div className="mt-4 space-y-2 text-xs">
                ...
                <ProgressBar
                  ...
                  color={worker.completedTasksToday ? "emerald" : "rose"}
                />
              </div>
            </Card>
          ))}
          ...
          <FormSelect
            ...
            onChange={(value) => {
              ...
              setForm((current) => ({
                ...current,
                buildingId: value,
                orgId: building?.orgId || current.orgId,
              }));
            }}
            options={[
              ...
              ...buildings.map((building) => ({
                value: building.id,
                label: `${building.name} (${building.city || "No city"})`,
              })),
            ]}
          />
          <FormInput
