...
function OwnerBuildings() {
  ...
  function load() {
    ...
    apiFetch<any>("/api/owner/overview")
      .then(setDashboard)
      .catch((error) =>
        showToast(error instanceof Error ? error.message : "Could not load buildings.", "error"),
      )
  ...
  const buildingLabour = selectedBuilding
    ? labour.filter((worker: any) => selectedBuilding.assignedLabourIds.includes(worker.id))
    : [];
  const buildingReports = selectedBuilding
    ? reports.filter((report: any) => report.buildingId === selectedBuilding.id).slice(0, 3)
    : [];
  const buildingLimitReached =
    usage.buildingLimit !== null &&
    usage.buildingLimit !== undefined &&
    usage.buildingsUsed >= usage.buildingLimit;
    ...
    <div className="space-y-6">
      ...
      {buildings.length === 0 ? (
        <Card>
          <EmptyState
            ...
            body="Add your first building to start receiving daily labour reports."
          />
        </Card>
      ) : (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {buildings.map((building: any) => {
            ...
            const assigned = labour.filter((worker: any) =>
              building.assignedLabourIds.includes(worker.id),
            );
            return (
              <button
                key={building.id}
                ...
                <div className="p-4">
                  <div className="mb-1.5 flex items-center justify-between text-xs">
                    <span className="font-medium text-muted-foreground">
                      {t("owner.buildings.today_completion", { fallback: "Today's completion" })}
                    </span>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      ...
                      {assigned.length
                        ? assigned.map((worker: any) => worker.name.split(" ")[0]).join(", ")
                        : t("owner.buildings.no_labour", { fallback: "No labour" })}
                    </div>
      ...
      {selectedBuilding && (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            <SectionTitle
              title={`${t("dashboard.owner.nav.labour", { fallback: "Labour" })} - ${selectedBuilding.name}`}
            />
            {buildingLabour.length === 0 ? (
              <div className="py-6 text-center text-sm text-muted-foreground">
                {t("owner.buildings.no_labour_assigned", {
                  fallback: "No labour assigned to this building.",
                })}
              </div>
            ) : (
          ...
          <Card>
            <SectionTitle
              title={`${t("owner.buildings.recent_reports", { fallback: "Recent Reports" })} - ${selectedBuilding.name}`}
            />
            {buildingReports.length === 0 ? (
              <div className="py-6 text-center text-sm text-muted-foreground">
                {t("owner.buildings.no_reports", { fallback: "No reports for this building yet." })}
              </div>
            ) : (
              <div className="space-y-3">
      ...
      <Modal
        ...
        title={t("admin.buildings.add", { fallback: "Add Building" })}
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Btn onClick={handleAdd} disabled={!form.name}>
              {t("admin.buildings.add", { fallback: "Add Building" })}
            </Btn>
          </div>
        </div>
      </Modal>
