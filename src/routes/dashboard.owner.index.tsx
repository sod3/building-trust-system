...
function OwnerOverview() {
  ...
  const { organization, subscription, plan, usage, kpis, buildings, labour, todayReports } =
    dashboard;
    ...
    <div className="space-y-6">
      <div>
        <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          {t("dashboard.owner.hub", { fallback: "Your Building Operations Hub" })}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
        <Kpi
          accent="navy"
          label={t("dashboard.owner.nav.buildings", { fallback: "My Buildings" })}
          value={usage.buildingsUsed}
          sub={`${plan?.name || "Plan"}: ${formatLimit(usage.buildingLimit)}`}
          icon={<Building2 className="h-4 w-4" />}
        />
        <Kpi
          accent="emerald"
          label={t("owner.overview.tasks_done", { fallback: "Tasks Completed Today" })}
          value={kpis.tasksCompletedToday}
          sub={`${t("common.of", { fallback: "of" })} ${kpis.tasksTotalToday} ${t("common.total", { fallback: "total" })}`}
          icon={<CheckCircle2 className="h-4 w-4" />}
        />
        <Kpi
          accent="gold"
          label={t("owner.overview.completion", { fallback: "Today Completion" })}
          value={`${kpis.completionRate}%`}
          sub={t("owner.overview.across_buildings", { fallback: "Across all buildings" })}
          icon={<TrendingUp className="h-4 w-4" />}
        />
        <Kpi
          label={t("owner.overview.active_labour", { fallback: "Active Labour" })}
          value={usage.labourUsersUsed}
          icon={<Users className="h-4 w-4" />}
          sub={`Limit: ${formatLimit(usage.labourLimit)}`}
        />
        <Kpi
          label={t("owner.overview.pending_tasks", { fallback: "Pending Tasks" })}
          value={kpis.pendingTasks}
          icon={<Clock className="h-4 w-4" />}
          sub={t("owner.overview.across_buildings", { fallback: "Across all buildings" })}
          tone={kpis.pendingTasks > 0 ? "down" : "up"}
        />
        <Kpi
          label="Next Billing"
          value={
      ...
      <Card>
        <SectionTitle
          ...
          action={
            <Link
              to="/dashboard/owner/buildings"
              className="text-xs text-accent hover:underline flex items-center gap-1"
            >
              {t("common.view_all", { fallback: "View all" })} <ArrowRight className="h-3 w-3" />
            </Link>
          }
        />
        {buildings.length === 0 ? (
          <EmptyState
            ...
            body="Add your first building to start tracking daily work."
          />
        ) : (
          <div className="space-y-3">
            {buildings.slice(0, 5).map((building) => (
              <div
                key={building.id}
                className="flex items-center gap-4 rounded-xl border border-border p-4 hover:bg-secondary/30 transition"
              >
                <img
                  src={building.cover}
                  alt={building.name}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {building.city || building.address || "No location set"}
                  </div>
                  <div className="mt-2">
                    <ProgressBar
                      ...
                      color={
                        building.completionToday >= 80
                          ? "emerald"
                          : building.completionToday >= 50
                            ? "amber"
                            : "rose"
                      }
                    />
                ...
                <div className="shrink-0 text-end space-y-1">
                  <div className="font-display text-lg font-semibold">
                    {building.completionToday}%
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {building.doneTasksToday}/{building.totalTasksToday}{" "}
                    {t("common.tasks", { fallback: "tasks" })}
                  </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <SectionTitle
            ...
            action={
              <Link
                to="/dashboard/owner/labour"
                className="text-xs text-accent hover:underline flex items-center gap-1"
              >
                {t("common.manage", { fallback: "Manage" })} <ArrowRight className="h-3 w-3" />
              </Link>
            }
          />
          {labour.length === 0 ? (
            <EmptyState
              ...
              title="No labour accounts"
              body="Create a labour login and assign it to a building."
            />
          ) : (
            <div className="space-y-3">
        ...
        <Card>
          ...
          {todayReports.length === 0 ? (
            <EmptyState
              ...
              body="Submitted labour reports will appear here immediately."
            />
          ) : (
            <div className="space-y-3">
                  ...
                  <div className="mt-1 flex items-center justify-between text-xs">
                    ...
                    {report.pendingTasks > 0 && (
                      <span className="text-amber-600 flex items-center gap-1">
                        ...
                        {report.pendingTasks} {t("common.pending", { fallback: "pending" })}
                      </span>
                    )}
                  </div>
                </div>
