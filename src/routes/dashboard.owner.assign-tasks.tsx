...
function OwnerAssignTasks() {
  ...
  function load() {
    apiFetch<any>("/api/owner/overview")
      ...
      .catch((error) =>
        showToast(error instanceof Error ? error.message : "Could not load templates.", "error"),
      );
  }
      ...
      {featureLocked && (
        <Card className="border-amber-200 bg-amber-50 text-amber-900">
          <div className="flex items-start gap-3 text-sm">
            ...
            <span>
              Checklist template creation is not included in Starter. Upgrade to Professional to
            ...
            {items.map((item, index) => (
              <div key={index} className="flex gap-2">
                ...
                <Btn
                  ...
                  disabled={items.length === 1 || featureLocked}
                >
          ...
          <div className="flex flex-wrap gap-2">
            <Btn
              ...
              onClick={() =>
                setItems((current) => [
                  ...current,
                  { title: "", icon: "CheckCircle2", required: true },
                ])
              }
              disabled={featureLocked}
            >
            ...
            <Btn
              ...
              disabled={featureLocked || !form.name || !items.some((item) => item.title.trim())}
            >
              Save Template
            </Btn>
          </div>
      ...
      <Card>
        ...
        {templates.length === 0 ? (
          <EmptyState
            ...
            body="Create your first template to guide labour daily reports."
          />
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {template.buildingId
                        ? buildings.find((building: any) => building.id === template.buildingId)
